 ...
 *
 * LeetCode question: https://leetcode.com/problems/longest-repeating-character-replacement/
 *
 */
export const findLongestRepeatingCharacterReplacement = (string: string, k: number): number => {
  let maxSize = 0, most = 0
  const countByChar = new Map()
  for (let fast = 0, slow = 0; fast < string.length; fast++) {
    ...
    if ((fast - slow + 1) - most <= k) {
      maxSize = Math.max(fast - slow + 1, maxSize)
      ...
      // Shrink Window
      while ((fast - slow + 1) - most > k && slow <= fast) {
        const slowChar = string[slow]
        const slowCount = countByChar.get(slowChar)

        const nextSlowCount = slowCount - 1
        countByChar.set(slowChar, nextSlowCount)

        if (slowCount === most) {
          for (const [_, value] of countByChar) {
            most = Math.max(value, most)
          }
        }
