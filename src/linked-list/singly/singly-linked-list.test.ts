...
} from './singly-linked-list.practice'
...
describe('Test removeNthNodeFromEndOfListUsingTwoPointer()', () => {
  ...
  it('Should_RemoveNthNodeFromEndOfList_WhenGivenValidInput', () => {
    ...
    expect(fourthLL.head).toBeTruthy()
    expect(fourthLL.find('d')).toBeFalsy()
    expect(fourthLL.find('c')?.next?.value).toBe('e')

    const fifLL = removeNthNodeFromEndOfListUsingTwoPointer(
...
describe('Test mergeKSortedList()', () => {

  it('Should_DoNothing_WhenGivenInvalidList', () => {
    const emptyLLs = mergeKSortedListByBruteForce([])
    expect(emptyLLs).toBeFalsy()

    const onlyOneLLs = mergeKSortedListByBruteForce([new SinglyLinkedList().append(1).append(2)])
    expect(onlyOneLLs.head?.value).toBe(1)
    expect(onlyOneLLs.head?.next?.value).toBe(2)
  })
